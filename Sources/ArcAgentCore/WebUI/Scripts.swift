...
///
/// ~120 lines of vanilla JS. Compiled into the binary as a static string.
/// Inlined in the HTML via `<script>` tag. No external files, no build step.
///
/// ## Capabilities
///
/// - **WebSocket client** — proper class with exponential backoff reconnection
...
/// - **Smart auto-scroll** — only auto-scrolls if user is near the bottom
...
/// - **Model switching** — change the active model
/// - **Settings panel** — slide-out drawer with temperature, max tokens, streaming toggle
/// - **Auto-resize textarea** — grows with content up to max-height
/// - **Connection status** — updates the `#conn` element on connect/disconnect
/// - **Message queuing** — queues messages sent while disconnected
public enum Scripts {
